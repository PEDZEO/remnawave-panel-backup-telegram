...
- Полный backup/restore Bedolaga-стека (бот + кабинет): PostgreSQL, Redis, `.env`, compose и рабочие данные.
- Восстановление из локального файла или по прямой ссылке (URL).
- SHA256 checksum для новых архивов и проверка checksum перед restore, если `.sha256` лежит рядом.
- Шифрование архивов резервной копии (GPG symmetric), чтобы хранить и передавать их безопаснее.
- Разделение Telegram-уведомлений по топикам: отдельно для backup панели и отдельно для backup Bedolaga (бот + кабинет).
- Отображение статуса: таймер, сервис, последняя резервная копия, состояние контейнеров и базовые диагностические данные.
- Doctor-проверка без изменений в системе: `MODE=doctor bash <(curl -fsSL https://raw.githubusercontent.com/PEDZEO/remnawave-panel-backup-telegram/main/install.sh)`.
